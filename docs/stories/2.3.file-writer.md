...
- [x] API Route: `web/src/app/api/orchestrator/file-write/route.ts` (POST) (AC: 5)
  ...
  - [x] Ghi thành công → gọi `POST /api/telemetry/event` với `{ event: "file_written", props: { path, bytes }, ts }`
...
| 2025-08-10 | 1.1     | Harden symlink trong fileWriter; cập nhật routes dùng await; smoke pass | James  |
...
- 2025-08-10: Bổ sung kiểm tra symlink trong `sanitizeAndResolvePath`; cập nhật routes dùng `await`; smoke `doc-out` (brief) pass.

## Completion Notes List

- AC1–AC8: ĐẠT; bảo vệ đường dẫn, ràng buộc phần mở rộng, telemetry gửi sau ghi.
- Bổ sung kiểm tra symlink; xác nhận suffix tăng dần, từ chối extension lạ và path traversal qua smoke test.

## File List

- `web/src/lib/fileWriter.ts` (new)
- `web/src/app/api/orchestrator/file-write/route.ts` (new)
- `web/src/app/api/orchestrator/doc-out/route.ts` (modified)
...
- Review kỹ thuật (security & correctness):
  - Security (Medium): ĐÃ SỬA — Telemetry URL chuyển sang `new URL("/api/telemetry/event", req.url)` thay vì dựa vào Host header.
  - Dry-run (Low): ĐÃ SỬA — `dryRunFileWrite` tính `resolveConflictSuffix` và trả `wouldWritePath` cuối cùng.
  - DX/Privacy (Low): ĐÃ SỬA — API trả về `path` tương đối so với repo root.
  - Defense in depth (Low): ĐÃ SỬA — `writeFileSafely` double-check extension trước khi ghi.
  - Optional: traversal check bằng `includes("..")` vẫn giữ vì đã có normalize + realpath guard; có thể nâng cấp sang `path.relative` trong tương lai nếu cần.

- Kết luận: Đạt Acceptance Criteria 1–8. Khuyến nghị hardening theo mục ở trên (không chặn release).
  - Sau hardening: vẫn ĐẠT; build/lint/type-check pass.

- Ghi chú kiểm chứng:
  - Lint/Typecheck/Build: `npm run lint`, `npm run build` → pass sau các chỉnh sửa hardening.
